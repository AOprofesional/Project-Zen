...
import { Loader2, ArrowLeft, CheckCircle, RefreshCcw, Send, FileUp, Info, MessageSquare } from 'lucide-react';
import { getClientActions, respondToAction } from '@/services/clientActions';
import { ClientRequestManager } from '@/components/clients/ClientRequestManager';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-2">
                <h1 className="text-3xl font-bold">Colaboración del Proyecto</h1>
                <p className="text-gray-400">Gestiona tus respuestas y realiza nuevas solicitudes al equipo.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                {/* LEFT COLUMN: PENDING FROM ADMIN */}
                <div className="space-y-6">
                    <h2 className="text-xl font-bold flex items-center gap-2 text-emerald-400">
                        <CheckCircle size={20} /> Pendientes de tu parte
                    </h2>
                            ...
                            {pendingOnClient.map(action => (
                                <Card key={action.id} className="p-6 border-white/5 bg-white/[0.02]">
                                    <div className="flex flex-col gap-6">
                                        ...
                                        <div className="space-y-4 pt-4 border-t border-white/5 leading-relaxed">
                                            <p className="text-xs font-medium text-gray-500 uppercase tracking-widest">Tu Respuesta</p>
                ...
                {/* RIGHT COLUMN: CLIENT REQUESTS */}
                <div className="space-y-6">
                    {projectId && <ClientRequestManager projectId={projectId} />}
                </div>
            </div>
